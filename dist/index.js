...
var require_dist_node11 = __commonJS({
  "node_modules/@octokit/graphql/node_modules/@octokit/endpoint/dist-node/index.js"(exports, module2) {
    "use strict";
...
var require_dist_node12 = __commonJS({
  "node_modules/@octokit/graphql/node_modules/@octokit/request-error/dist-node/index.js"(exports, module2) {
    "use strict";
...
var require_dist_node13 = __commonJS({
  "node_modules/@octokit/graphql/node_modules/@octokit/request/dist-node/index.js"(exports, module2) {
    "use strict";
    var __defProp2 = Object.defineProperty;
    var __getOwnPropDesc2 = Object.getOwnPropertyDescriptor;
    var __getOwnPropNames2 = Object.getOwnPropertyNames;
...
async function getCommitMessagesFromPullRequest(accessToken, repositoryOwner, repositoryName, pullRequestNumber) {
  const { repository } = await (0, import_graphql.graphql)(query, {
...
// src/input.ts
var validFlagRe = /[^gimsuy]/g;
var truth = ["true", "True", "TRUE"];
async function getInputs() {
  const pattern = core2.getInput("pattern", { required: true });
  const error = core2.getInput("error", { required: true });
  const flags = core2.getInput("flags");
  if (pattern.length === 0) {
  ...
  return {
    ...
    messages: await getMessages({
      checkTitle: !truth.includes(core2.getInput("excludeTitle")),
      checkDescription: !truth.includes(core2.getInput("excludeDescription")),
      checkAllCommitMessages: truth.includes(core2.getInput("checkAllCommitMessages")),
      accessToken: core2.getInput("accessToken")
    })
  };
}
async function handlePullRequst(options, context2) {
  if (!context2.payload) {
  ...
  if (options.checkAllCommitMessages) {
    ...
    if (!github.context.payload.repository.owner || !github.context.payload.repository.owner.login && !github.context.payload.repository.owner.name) {
...
async function handlePush(options, context2) {
  if (!context2.payload) {
    throw new Error("No payload found in the context.");
  }
  if (!Array.isArray(github.context.payload.commits)) {
    core2.debug("No commits -- skipping");
    return [];
  }
  return context2.payload.commits.filter((v) => v && !!v.message).map(({ message }) => message);
}
var handlers = {
...
async function getMessages(options) {
  core2.debug(`Getting messages ${github.context.eventName}`);
  return handlers[github.context.eventName]?.(options, github.context) ?? [];
}

// src/checker.ts
var core3 = __toESM(require_core());
function checkCommitMessages({ messages, patternRe, error }) {
  if (messages.length === 0) {
    core3.info("No messages to check");
    return;
  }
  core3.info(`Checking commit messages against: ${String(patternRe)}`);
  const result = messages.every((message) => {
    if (!patternRe.test(message)) {
      core3.info(`failed: ${message}`);
      return false;
    }
    return true;
  });
...
async function main() {
  try {
    const args = await getInputs();
    if (args.messages.length === 0) {
      core4.info(`No commits found in the payload, skipping check.`);
      return;
    }
    checkCommitMessages(args);
  } catch (error) {
